...
- When a turn ends with `exit_plan_mode`, the desktop completion notification now renders as a distinct `Plan ready` toast with the plan title as the body and a separate `omp-plan-<sessionId>` group key (instead of the generic `Task complete`). The distinct group means plan-ready toasts and regular completions don't collapse each other in Notification Center — the user can review both kinds independently.
