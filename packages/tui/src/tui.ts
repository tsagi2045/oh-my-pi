...
export class TUI extends Container {
	...
	#doRender(): void {
		...
		// Soft full-render of just the visible viewport: cursor home + the last
		// `height` content rows + erase-after-content. Used when a change above
		// the previous viewport top would otherwise force `fullRender(true)`.
		//
		...
		//
		// IMPORTANT: this MUST write at most `height` lines. The previous
		// implementation wrote every line in `newLines`, which on terminals
		// without alt-screen (OMP runs on the main screen) pushed
		// `newLines.length - height` rows into scrollback on every refresh.
		// With `PI_DEBUG_REDRAW=1` on a 24-row terminal and a 52-line transcript
		// that fired every 30ms during streaming, the user saw historical
		// prompt frames stacking up in scrollback — even though the visible
		// viewport rendered correctly. Anchoring the repaint window to the
		// bottom of `newLines` (last `height` rows) keeps cursor-home + line
		// writes strictly inside the visible viewport, so no scroll occurs and
		// no scrollback rows are touched.
		//
		// Lines wider than `width` are truncated to prevent terminal auto-wrap
		// from pushing content past the bottom row and re-introducing the same
		// scroll. Image lines are passed through (their geometry is handled by
		// the image protocol).
		//
		// Scrollback above the viewport may still show stale rendering of older
		// content — but only the SAME lines that were already there, since we
		// no longer push fresh copies. The next non-soft frame fully reconciles.
