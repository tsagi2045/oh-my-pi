...
- Reworked desktop notification dispatch around an explicit `LegacyNotifier.deliveryOverride` field (also surfaced as `TerminalInfo.deliveryOverride`) and a new `setNotificationDelivery("auto" | "alerter" | "osc")` setter on the TERMINAL singleton. `notify.delivery` in the coding-agent settings flows into this at boot, letting ghostty / iTerm2 / wezterm users opt into the alerter shell-out path (toast icon becomes alerter's `>_` but click-jump back to the originating tmux pane becomes available) without having to lose the native LSApplication identity unilaterally. Default stays `auto` — current native-vs-shell-out heuristic for everyone who doesn't set the override.
- Rewrote `composeNotificationSubtitle` to emit `<windowName> · <ompSessionName>` — second half is the per-OMP-run label (e.g. `"Ghostty tmux Oh-My-Posh 알림 설정"`), NOT the tmux session name. Fallback chain: windowName + ompSession both → joined, either alone, then tmux sessionName as a last-resort. The earlier coordinate form `<session>:<window#>.<pane#> · <label>` and the v2 `<windowName> · <tmuxSessionName>` form were both reverted after the user flagged them: the tmux session name is just a workspace identifier shared across many OMP runs, while the OMP session name is the meaningful per-run work label. `TmuxFocusAction` no longer carries `windowIndex` / `paneIndex` (they had no other consumer); `terminalApp` is unchanged.
- Added `TerminalInfo.macAppName` (e.g. `"Ghostty"`, `"iTerm"`, `"WezTerm"`, `"kitty"`, `"Alacritty"`) so the click handler can `osascript -e 'tell application "<name>" to activate'` against the correct outer terminal. Fire sites stamp it onto `onClick.terminalApp`.
- Rewrote `scripts/notify-click.sh`: positional signature is now `<terminal_app> <session> <window> <pane>`. Activates the named macOS app generically (the previous version hard-coded `tell application "kitty"`), runs the kitty-socket tab-match step only when terminal_app is literally `kitty`, then does one static pane-background flip (default `colour237` held for 1.2 s — muted dark-gray, single brief shade rather than a strobing blink) and resets. Tunable via `OMP_PANE_BLINK_COLOR` / `OMP_PANE_BLINK_HOLD_SECONDS`. Replaces the previous border + background animation per user feedback ("테두리 깜빡임 불필요, 배경 한 번 부드럽게").
- Updated `scripts/mac-alerter.sh` to forward the new `<terminal_app>` positional through to `notify-click.sh`.
- Reversed `findMacNotifier` priority so `terminal-notifier` is preferred over `alerter`. On macOS Tahoe (26.x) alerter's `--sender com.apple.Terminal` impersonation is ineffective — macOS attributes the toast to the nearest GUI parent process instead, which for `omp` inside ghostty's tmux is **Ghostty itself**, causing the notification to inherit Ghostty's "Persistent / Alert" notification style and never auto-dismiss. `terminal-notifier` ships its own `.app` bundle (`fr.julienxx.oss.terminal-notifier`) so macOS attributes the toast to it directly, the user gets a real Banner with macOS-controlled auto-dismiss, and Notification Center groups entries under "terminal-notifier". `alerter` stays as a fallback when only it is installed, but emits a one-shot `logger.warn` nudging the user to `brew install terminal-notifier` on first use.
- Added `tmux switch-client -t <session>` to `scripts/notify-click.sh` before `select-window`. The previous version only ran `select-window -t <session>:<window>`, which silently moves the window pointer of the OTHER session in the background but does NOT flip the attached client's active session — so when the user clicked a notification from a different tmux session they saw nothing change. `switch-client` lands the attached client on the originating session first; `select-window` + `select-pane` then run where the user can actually see them.
- Added Ghostty AppleScript `select tab` step to `scripts/notify-click.sh` (runs before `tmux switch-client`). On user setups where each tmux session lives in its own Ghostty tab (the `⌘1` / `⌘2` / `⌘3` shortcut tabs), `switch-client` alone flipped some background client's active session but did NOT touch the Ghostty tab the user was looking at. The AppleScript step iterates Ghostty's windows + tabs, finds the tab whose title starts with `<session> ` (matching the standard `tmux set-titles-string '#S — #W'` convention), and selects it — so the user lands in the correct tab + session + window + pane in one click. Falls through silently when the AppleScript dictionary isn't responsive (e.g. older Ghostty).

### Removed

- Removed `scripts/notify-flash.sh` and `flashOriginatingPane()` from `notify/desktop.ts`. The 8-second border-gold hold on the native-darwin OSC path was a workaround for the OSC 9 click-callback hole; users who care about click feedback now flip `notify.delivery` to `"alerter"` and get a real click handler (with pane-background flash). Users who keep `auto` see no flash at all on the native path, which matches the user-stated preference.
- Removed `excerptAssistantMessage` / `excerptAskPrompt` 80/60-character clips. Both now use a 200-character limit so the notification body carries the agent's actual answer / question text within the macOS toast width budget (was: aggressive single-line ellipsis).
### Breaking Changes
...
- Added a `nativeMacosNotifications` field on `TerminalInfo` (and the `LegacyNotifier` interface) — `true` for ghostty / iTerm2 / wezterm because each is its own registered macOS `LSApplication` with notification permission. `sendDesktopNotification` now consults this flag on darwin: native-on-darwin terminals get the OSC 9 / OSC 99 escape directly (terminal's bundle owns the notification, including auto-dismiss + Notification Center archival + click-to-focus), while kitty / alacritty / vscode and unknown terminals continue through the `alerter` / `terminal-notifier` fallback.
- Added `parseTmuxEnvForOuterTerminal()` and an internal `tmux show-environment -g` probe in the `TERMINAL_ID` resolver. When `TERM_PROGRAM=tmux` masks the outer terminal (because tmux only forwards a fixed allowlist of env vars to shells), the probe reads tmux's own inherited environment for marker variables (`KITTY_WINDOW_ID`, `GHOSTTY_RESOURCES_DIR`, `WEZTERM_PANE`, `ITERM_SESSION_ID`, `ALACRITTY_WINDOW_ID`) so OMP identifies the real outer terminal and routes notifications correctly. Synchronous, one subprocess at startup, falls through to the existing `COLORTERM` / `base` defaults when tmux isn't reachable.
- Added `scripts/notify-flash.sh` and `flashOriginatingPane()` in `notify/desktop.ts`. When `sendDesktopNotification` takes the native-darwin OSC path (ghostty / iTerm2 / wezterm), the terminal app consumes the click in-app — OMP never sees it, so the alerter-path `notify-click.sh` flash can't run. The new helper fires at dispatch time and sets the originating tmux pane border to gold for 8 s (override via `OMP_FLASH_HOLD_SECONDS`, `0` to disable) via tmux IPC, so the user can still spot the pane after returning to the terminal. Static color change only — no background blinking, no select-pane, no animation, because the earlier design's background pulses stacked into a distracting flicker across consecutive notifications during normal code editing. Fire-and-forget, no escape sequences emitted, skipped silently outside tmux.
- Added `parseTmuxClientTermname()` and an additional `tmux list-clients -F '#{client_termname}'` probe in `TERMINAL_ID` resolution. Reads the TERM value of the currently-attached client (e.g. `xterm-ghostty`) — authoritative for "which terminal is OMP's OSC output reaching right now" and tolerant of the case where the tmux server was started under a different terminal than the one currently displaying it.
