...
- Added `scripts/notify-flash.sh` and `flashOriginatingPane()` in `notify/desktop.ts`. When `sendDesktopNotification` takes the native-darwin OSC path (ghostty / iTerm2 / wezterm) the terminal app consumes the click in-app — OMP never sees it, so the alerter-path `notify-click.sh` flash can't run. The new helper fires at dispatch time, sets the originating tmux pane border to gold via tmux IPC, does two quick 150 ms background blinks to draw the eye to the pane simultaneously with the toast appearing in the corner, and holds the golden border for ~30 s (override via `OMP_FLASH_HOLD_SECONDS`) so the user can still spot the pane after returning to the terminal. Fire-and-forget, no escape sequences emitted, skipped silently outside tmux.
