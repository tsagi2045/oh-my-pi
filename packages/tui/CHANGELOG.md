...
- Fixed alerter desktop notifications disappearing after ~10 s and not persisting to macOS Notification Center. `scripts/mac-alerter.sh` previously passed `--timeout 30`, which makes alerter call `removeDeliveredNotification` and purge the entry from NC entirely; the `--actions "Open"` flag was also conditional on a click target. We now pass `--actions "Open"` unconditionally (alerter renders Alert-style, which both keeps the toast on screen until the user closes it AND archives it to NC) and drop the explicit timeout so missed notifications remain reviewable later.
